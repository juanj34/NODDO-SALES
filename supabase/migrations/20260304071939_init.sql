...
-- Owner full access for child tables
CREATE POLICY "Owner full access tipologias"
  ON tipologias FOR ALL
  USING (EXISTS (SELECT 1 FROM proyectos WHERE id = proyecto_id AND auth.uid() = user_id));

CREATE POLICY "Owner full access galeria_categorias"
  ON galeria_categorias FOR ALL
  USING (EXISTS (SELECT 1 FROM proyectos WHERE id = proyecto_id AND auth.uid() = user_id));

CREATE POLICY "Owner full access galeria_imagenes"
  ON galeria_imagenes FOR ALL
  USING (EXISTS (SELECT 1 FROM galeria_categorias gc JOIN proyectos p ON gc.proyecto_id = p.id WHERE gc.id = categoria_id AND auth.uid() = p.user_id));

CREATE POLICY "Owner full access videos"
  ON videos FOR ALL
  USING (EXISTS (SELECT 1 FROM proyectos WHERE id = proyecto_id AND auth.uid() = user_id));
...
-- Storage policies: authenticated users can upload, anyone can read public
