const site = {
  landing: {
    enterExperience: "ENTRAR A EXPERIENCIA",
  },
  tipologias: {
    features: "Características",
    jacuzzi: "Jacuzzi",
    piscina: "Piscina",
    bbq: "BBQ privado",
    terraza: "Terraza privada",
    jardin: "Jardín privado",
    cuartoServicio: "Cuarto de servicio",
    estudio: "Estudio",
    chimenea: "Chimenea",
    dobleAltura: "Doble altura",
    rooftop: "Rooftop privado",
    yes: "Sí",
    lessInfo: "Menos",
    moreInfo: "Info",
    units: "Unidades",
    available: "disponibles",
    noUnits: "No hay unidades",
    floor: "Piso",
    enquire: "COTIZAR",
    closeImage: "Cerrar imagen",
    closeDetail: "Cerrar detalle",
    notAvailable: "Tipologías no disponibles",
    notConfigured: "Aún no se han configurado tipologías para este proyecto.",
    noTorreTipologias: "Sin tipologías en esta torre",
    noTorreTipologiasDesc: "Aún no se han asignado tipologías a esta torre.",
    areas: "Áreas",
    internalArea: "Interna",
    balcony: "Balcón",
    areaConstruida: "Construida",
    areaPrivada: "Privada",
    areaLote: "Lote",
    total: "Total",
    from: "Desde",
    studio: "Studio",
    comercial: "Comercial",
    location: "Ubicación",
    locationInProject: "Ubicación en el proyecto",
    renders: "Renders",
    image: "imagen",
    images: "imágenes",
    verVista: "Ver Vista",
    terrain: "Terreno",
    construction: "Construcción",
    priceBreakdown: "Desglose",
    watchVideo: "Ver Video",
    viewTour360: "Tour 360°",
  },
  galeria: {
    prevImage: "Imagen anterior",
    nextImage: "Imagen siguiente",
    fullscreen: "Ver pantalla completa",
    categoryNotFound: "Categoría no encontrada",
    gallery: "Galería",
    notAvailable: "Galería no disponible",
    notConfigured: "Aún no se han agregado imágenes a la galería de este proyecto.",
    general: "General",
  },
  ubicacion: {
    notAvailable: "Ubicación no disponible",
    satellite: "Satélite",
    map: "Mapa",
    render: "Render",
    openGoogleMaps: "ABRIR EN GOOGLE MAPS",
    pointsOfInterest: "Puntos de Interés",
    nearbyDestinations: "destinos cercanos",
    all: "Todos",
    noPoisCategory: "No hay puntos de interés en esta categoría",
    getDirections: "Cómo Llegar",
  },
  contacto: {
    sectionLabel: "Contacto",
    heading: "Hablemos de {{name}}",
    description: "Completa el formulario y un asesor especializado te contactará.",
    valueProp1Title: "Respuesta rápida",
    valueProp1Desc: "Te contactamos en menos de 24 horas",
    valueProp2Title: "Datos protegidos",
    valueProp2Desc: "Tu información está segura con nosotros",
    valueProp3Title: "Asesor dedicado",
    valueProp3Desc: "Atención personalizada para tu inversión",
    trustLine: "Tus datos están protegidos y no serán compartidos",
    consentPrefix: "Al enviar, aceptas nuestra",
    consentLink: "política de privacidad",
    successHeading: "¡Mensaje enviado!",
    successDescription: "Un asesor especializado te contactará pronto para resolver todas tus dudas sobre {{name}}.",
    successNext: "Revisa tu correo para la confirmación",
  },
  brochure: {
    sectionLabel: "Brochure",
    heading: "Descarga nuestro brochure",
    description: "Conoce todos los detalles del proyecto {{name}}, tipologías, acabados y mucho más.",
    downloadPdf: "DESCARGAR PDF",
    pageOf: "de",
    loadingPdf: "Cargando brochure...",
    errorLoading: "No se pudo cargar el brochure",
    notAvailable: "Brochure no disponible",
    notConfigured: "El brochure aún no ha sido cargado para este proyecto.",
  },
  tour360: {
    sectionLabel: "Tour 360°",
    notAvailable: "Tour virtual no disponible",
    notConfigured: "El tour virtual aún no ha sido configurado para este proyecto.",
  },
  videos: {
    prevVideos: "Videos anteriores",
    nextVideos: "Más videos",
    notAvailable: "Videos no disponibles",
    notConfigured: "Aún no se han agregado videos a este proyecto.",
  },
  explorar: {
    exploreTower: "Explorar {{name}}",
    exploreFachada: "Explorar {{name}}",
    exploreFachadas: "Explorar Fachadas",
    fachada: "fachada",
    fachadas: "fachadas",
    selectUnit: "Selecciona una unidad",
    selectUnitDescription: "Selecciona una unidad para ver sus detalles",
    towers: "Torres",
    fachadasLabel: "Fachadas",
    detailTitle: "Detalle de Unidad",
    closeDetail: "Cerrar detalle",
    area: "Area",
    floor: "Piso",
    bedrooms: "Habitaciones",
    bathrooms: "Baños",
    parking: "Parqueaderos",
    storage: "Depósitos",
    studio: "Studio",
    view: "Vista",
    verVista: "Ver Vista",
    price: "Precio",
    enquireUnit: "COTIZAR UNIDAD",
    moreInfo: "Más Información",
    backToImplantaciones: "Volver a implantaciones",
    pisos: "Pisos",
    fachadaView: "Vista fachada",
    plantaView: "Vista planta",
    explorePlantas: "Explorar Plantas",
    noUnits: "No hay unidades en esta fachada",
    implantacion: "Implantación",
    expandImplantacion: "Ampliar implantación",
    selectTower: "Selecciona una torre para explorar",
    notAvailable: "Explorador no disponible",
    notConfigured: "Aún no se han configurado fachadas para este proyecto.",
  },
  inventario: {
    heading: "Inventario",
    tabApartamentos: "Apartamentos",
    tabCasas: "Casas",
    tabLotes: "Lotes",
    tabLocales: "Locales",
    search: "Buscar...",
    typeAll: "Tipo: Todas",
    allBedrooms: "All",
    studioShort: "St",
    sortFloorAsc: "Piso ↑",
    sortFloorDesc: "Piso ↓",
    sortPriceAsc: "Precio ↑",
    sortPriceDesc: "Precio ↓",
    sortAreaAsc: "Área ↑",
    sortAreaDesc: "Área ↓",
    gridView: "Vista cuadrícula",
    listView: "Vista lista",
    result: "resultado",
    results: "resultados",
    unit: "Unidad",
    type: "Tipología",
    area: "Área",
    bedrooms: "Hab",
    bathrooms: "Baños",
    parking: "Parq.",
    storage: "Dep.",
    orientation: "Orient.",
    view: "Vista",
    floor: "Piso",
    price: "Precio",
    viewUnit: "Ver unidad {{unit}}",
    enquireUnit: "Cotizar unidad {{unit}}",
    noResults: "Sin resultados",
    noResultsDescription: "No hay unidades que coincidan con los filtros seleccionados. Intenta ajustar los criterios de búsqueda.",
    clearFilters: "LIMPIAR FILTROS",
    etapaAll: "Etapa: Todas",
    etapa: "Etapa",
    available: "disp.",
    chooseTipologia: "Elige una tipología",
    notAvailable: "Inventario no disponible",
    viewFloorPlan: "Ver plano",
    notConfigured: "Aún no se han agregado unidades al inventario de este proyecto.",
  },
  implantaciones: {
    heading: "Implantación del proyecto",
    selectPoint: "Selecciona una torre para explorar",
    notAvailable: "Implantaciones no disponibles",
    notConfigured: "Aún no se han configurado implantaciones para este proyecto.",
    closeDetail: "Cerrar detalle",
    description: "Descripción",
    viewItem: "Ver {{name}}",
    plans: "Planos",
    hotspots: "Puntos de interés",
    hotspotCount: "{{count}} punto(s)",
    viewRender: "Ver render",
    closeRender: "Cerrar render",
    noRender: "Sin render disponible",
    noImage: "Imagen del plano no disponible",
    loadTimeout: "La imagen tardó demasiado en cargar",
    retry: "Reintentar",
  },
  recursos: {
    heading: "Recursos",
    description: "Documentos y materiales descargables de {{name}}",
    view: "VER",
    download: "DESCARGAR",
    notAvailable: "Recursos no disponibles",
    notConfigured: "Aún no se han agregado recursos descargables a este proyecto.",
    types: {
      brochure: "Brochure",
      ficha_tecnica: "Ficha Técnica",
      acabados: "Acabados",
      lista_precios: "Lista de Precios",
      planos: "Planos",
      render: "Renders",
      manual: "Manual",
      reglamento: "Reglamento",
      garantias: "Garantías",
      otro: "Otro",
    },
  },
  avances: {
    heading: "Avances de Obra",
    subtitle: "Seguimiento del progreso de construcción",
    close: "Cerrar",
    watchVideo: "Ver video",
    viewDetails: "Ver detalles",
    notAvailable: "Avances no disponibles",
    notConfigured: "Aún no se han publicado avances de obra para este proyecto.",
  },
  planPago: {
    sectionLabel: "PLAN DE PAGOS",
    heading: "Plan de Pagos",
    subtitle: "Selecciona una unidad para ver el desglose completo de pagos",
    selectUnit: "Seleccionar unidad",
    total: "Total",
    installments: "cuotas de",
    frequency: {
      unica: "Pago único",
      mensual: "Mensual",
      bimestral: "Bimestral",
      trimestral: "Trimestral",
    },
    notAvailable: "Plan de pagos no disponible",
    notConfigured: "Aún no se ha configurado un plan de pagos para este proyecto.",
    noUnitsAvailable: "Sin unidades disponibles",
    noUnitsDescription: "No hay unidades con precios disponibles para el simulador de pagos.",
    selectPrompt: "Selecciona una unidad para ver su plan de pagos",
    cuotaInicial: "Cuota inicial",
  },
  cotizador: {
    heading: "Cotizar Unidad",
    subtitle: "Completa tus datos para recibir la cotización",
    subtitleCotizador: "Revisa el plan de pagos y recibe tu cotización por correo",
    subtitleLead: "Déjanos tus datos y un asesor te contactará",
    hab: "hab",
    banos: "baños",
    floor: "Piso",
    paymentPlan: "Plan de pagos",
    installmentsOf: "cuotas de",
    receiveByEmail: "Recibe tu cotización por correo",
    submitQuote: "Enviar cotización",
    quoteSent: "Cotización enviada",
    requestSent: "Solicitud recibida",
    checkEmailPdf: "Revisa tu correo para ver el PDF con el plan de pagos completo.",
    checkEmailLead: "Un asesor especializado te contactará en las próximas 24 horas.",
    downloadPdf: "Descargar PDF",
    priceBreakdown: "Desglose de precio",
    terrain: "Terreno",
    construction: "Construcción",
    whatsappFollowUp: "Contactar por WhatsApp",
    whatHappensNext: "¿Qué sigue?",
    nextStep1Quote: "Recibes la cotización en tu correo",
    nextStep1Lead: "Recibimos tu solicitud",
    nextStep2: "Un asesor te contactará en menos de 24 horas",
    nextStep3: "Agenda una visita al proyecto",
    pdfIncludes: "El PDF incluye: detalles de la unidad, plan de pagos y contacto del asesor",
  },
  whatsapp: {
    defaultMessage: "Hola, me interesa saber más sobre el proyecto",
  },
  mobile: {
    showPois: "Puntos de interés",
    showUnits: "Unidades",
    showHotspots: "Zonas",
  },
  trustBadges: {
    dataEncrypted: "Datos encriptados",
    fastResponse: "Respuesta <24h",
    guaranteed: "Garantizado",
    familiesTrust: "500+ familias confían",
    certified: "Certificado",
    noCommitment: "Asesoría gratuita",
    freePDF: "PDF personalizado",
    fiveMin: "En 5 minutos",
    flexiblePlans: "Planes flexibles",
    personalizedQuote: "Cotización personalizada",
    byAdvisor: "Preparada por un asesor",
  },
  timeline: {
    today: "Hoy",
    tomorrow: "Mañana",
    thisWeek: "Esta semana",
    nextWeek: "Próxima semana",
    now: "Ahora",
    fiveMin: "5 minutos",
    step1: "Envías tu consulta",
    step2: "Asesor te contacta",
    step3: "Visita agendada",
    step4: "Recibes cotización",
  },
  leadForm: {
    step1: {
      label: "Interés",
      title: "¿Qué te interesa?",
      subtitle: "Selecciona el tipo de propiedad que buscas",
    },
    step2: {
      label: "Contacto",
      title: "Tu información",
      subtitle: "Para que un asesor te contacte",
    },
    step3: {
      label: "Detalles",
      title: "Detalles adicionales",
      subtitle: "Cuéntanos más sobre tu búsqueda",
    },
    continue: "Continuar",
    back: "Atrás",
    submit: "Enviar consulta",
  },
  cotizadorForm: {
    step1: {
      label: "Datos",
      title: "Tu información",
      subtitle: "Para enviarte la cotización personalizada",
      subtitleLead: "Para que un asesor te contacte",
    },
    step2: {
      label: "Resumen",
      title: "Plan de pagos",
      titleLead: "Tu solicitud",
      subtitle: "Revisa el detalle antes de enviar",
      subtitleLead: "Confirma los datos y envía",
    },
    continue: "Ver cotización",
    continueLead: "Continuar",
    back: "Atrás",
    submit: "Enviar cotización",
    submitLead: "Solicitar información",
  },
} as const;

export default site;
