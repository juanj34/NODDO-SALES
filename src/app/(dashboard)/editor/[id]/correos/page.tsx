...
export default function CorreosPage() {
  ...
  const handleAIImprove = useCallback(
    async (field: "saludo" | "cuerpo" | "despedida") => {
      ...
      if (!text?.trim()) {
        toast.error("Escribe algo primero para poder mejorar el texto");
        return;
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          toast.error(err?.error || "Error al mejorar el texto");
        }
      } catch {
    ...
    [config, project?.idioma, updateConfig, toast]
  );
  ...
  return (
    ...
    >
      <PageHeader
        icon={Mail}
        title={ts("correos.title")}
        description={ts("correos.description")}
      />
