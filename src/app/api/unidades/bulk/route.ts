  ...
  try {
    ...
    const rows = unidades.map(
      (u: Record<string, unknown>, i: number) => ({
        ...pick(u, ["tipologia_id", "identificador", "piso", "area_m2", "precio", "estado", "habitaciones", "banos", "orientacion", "vista", "notas", "fachada_id", "fachada_x", "fachada_y", "planta_id", "planta_x", "planta_y", "torre_id", "parqueaderos", "depositos"]),
        proyecto_id,
        orden: u.orden ?? i,
      })
    );
